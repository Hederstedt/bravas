...
- [x] **Valheim i BVS-betyget:** Steam exponerar varken räknare eller achievements för Valheim (se "Valheim-statistik, del 2" nedan), så den enda per-gubbe-signalen som finns är livstidsspeltiden vi redan cachar i `valheim_playtime`. Den blir nu en tredje betygskälla i `playerCards.ts`, med samma "aldrig ett avdrag"-regel som WoT — rent deltagande, ingen skicklighet mäts, så den som varken spelar CS2 eller WoT slipper stå kvar på "OKÄND" bara för att spelvalet är ett annat. Basen väljs i ordningen CS2 → WoT → Valheim, och varje ytterligare källa lägger på samma kapade bonus (`bonusFrom`, delad mellan WoT och Valheim). Ingen ny infrastruktur — playtimen hämtades redan för "Mest speltid i Valheim"-highlighten.
